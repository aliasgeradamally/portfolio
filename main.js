...
if (workMain) {
  ...
  if (p) {
    ...
    const links = p.links ? `<div class="project-links">${p.links.map(([label,url])=>`<a href="${url}" target="_blank" rel="noreferrer">${label} <b><svg class="site-arrow" viewBox="0 0 24 24" aria-hidden="true" focusable="false"><use href="/assets/icons.svg#arrow-up-right"></use></svg></b></a>`).join('')}</div>` : '';
    const resource = p.resource ? `<a class="project-resource" href="${p.resource.href}" target="_blank" rel="noreferrer"><img src="${p.resource.image}" alt="Kalam 25/26 e-booklet poster"><span>${p.resource.label} <b><svg class="site-arrow" viewBox="0 0 24 24" aria-hidden="true" focusable="false"><use href="/assets/icons.svg#arrow-up-right"></use></svg></b></span></a>` : '';
    workMain.innerHTML = `<section class="project-hero ${p.accent}"><div><p class="kicker">${p.type}</p><h1>${p.title}</h1><p class="lede">${p.intro}</p><p class="project-role">${p.role}</p>${links}${resource}</div><div class="project-hero-image"><img src="${p.hero}" alt="${p.title.replace(/<[^>]+>/g,'')}"></div></section><section class="project-note"><p class="kicker">The approach</p><h2>${p.note}</h2></section>${p.images.length ? `<section class="project-gallery ${p.gallery || ''} ${p.images.length === 1 ? 'one' : ''}">${p.images.map((src,i)=>`<figure class="gallery-${i+1}"><img src="${src}" alt="${p.title.replace(/<[^>]+>/g,'')} detail ${i+1}"></figure>`).join('')}</section>` : ''}<section class="next-project"><p class="kicker">Keep exploring</p><a href="/work/">All selected work <b><svg class="site-arrow" viewBox="0 0 24 24" aria-hidden="true" focusable="false"><use href="/assets/icons.svg#arrow-up-right"></use></svg></b></a></section>`;
  }
}
